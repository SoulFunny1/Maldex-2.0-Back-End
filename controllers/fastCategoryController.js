// controllers/fastCategoryController.js

// !!! АДАПТИРУЙТЕ ИМПОРТ !!!
// 1. Предполагаем, что FastCategory.js - это ваша модель:
import FastCategoryModel from "../models/fastCategory.js"; 
// 2. Предполагаем, что config/db.js экспортирует объект Sequelize:
import sequelize from "../config/db.js"; 

// Инициализируем модель, передавая ей экземпляр Sequelize
const FastCategory = FastCategoryModel(sequelize);
...
export const getAllCategories = async (req, res) => {
    try {
        const categories = await FastCategory.findAll({
            order: [["name", "ASC"]],
        });
...
export const getCategoryById = async (req, res) => {
    try {
        const category = await FastCategory.findByPk(req.params.id);
...
export const createCategory = async (req, res) => {
    try {
        ...
        const newCategory = await FastCategory.create(req.body);
        res.status(201).json(newCategory);
    } catch (error) {
        if (error.name === "SequelizeUniqueConstraintError") {
            return res
                .status(409) // Используем 409 Conflict для уникальности
...
export const updateCategory = async (req, res) => {
    try {
        const [updatedRows] = await FastCategory.update(req.body, {
        ...
        const updatedCategory = await FastCategory.findByPk(req.params.id);
...
export const deleteCategory = async (req, res) => {
    try {
        const deletedRowCount = await FastCategory.destroy({
            where: { id: req.params.id },
        });
